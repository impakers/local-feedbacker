...
// =============================================================================
// Impakers orbit — FAB 아이콘
// =============================================================================
//
// 임패커스 로고(impakers logo.svg)에서 워드마크를 감싸는 hairline 타원을 가져왔다.
// 로고 비율은 semi-minor/semi-major ≈ 0.35, 기울기 약 -20°.
// 중앙 도트는 "이 지점에 피드백을 남긴다"는 의미이고, 메뉴가 펼쳐지면 ✕로 바뀐다.
//
// 궤도는 **두 겹을 서로 다른 각도로 겹쳐서** 그린다(-24°/-45°). 각도는
// **비대칭**이어야 한다 — ±24° 같은 대칭 교차는 두 타원이 도트를 좌우 대칭으로
// 감싸서, 한 겹일 때와 똑같이 눈(eye) 아이콘처럼 보인다.
//
// 선은 **중앙에 가까운 옆구리로 갈수록 가늘어지고 장축 꼭짓점에서 가장 두껍다.**
// SVG stroke-width는 선 전체에 균일하게 적용돼 테이퍼를 만들 수 없어서,
// 폭이 변하는 **채움 경로**를 계산해 그린다.
//
// 모션은 은하 원반이 도는 방식을 따른다. **아이콘 전체를 360° 돌리지 않는다** —
// 궤도선의 자리는 그대로 두고, 그 선을 따라 별빛이 흐른다(dash가 둘레만큼 밀린다).
// 두 겹은 서로 다른 주기로 흘러서(4.2s / 6.8s) 위상이 계속 어긋난다 — 은하의
// 차등회전(안쪽이 빠르고 바깥이 느린)에서 온 인상이고, 덕분에 같은 장면이 반복되지 않는다.
//
// 별빛은 **항상 흐른다**. 궤도선 바로 안쪽(검은 면 위)을 지나가서 흰색 그대로
// 읽힌다(FLOW_TRACK).
//
// 쉴 때는 원반이 가라앉고(dim) 별빛이 느리게 돌며 겹이 3D로 흔들린다 — 멀리서
// 도는 은하다. hover하면 원반에 불이 들어오고(원래 밝기) 별빛이 두 배로 빨라지며
// 흔들림은 멎는다. 속도는 CSS duration이 아니라 JS의 playbackRate로 바꾼다
// (fab-menu/index.tsx). duration을 갈면 진행률이 `경과시간 / duration`으로 다시
// 계산돼 혜성이 순간이동하기 때문이다.
//
// hover 상태 전환은 FAB 버튼 쪽 CSS에서 `data-orbit-*` 속성으로 건다 —
// CSS 모듈 클래스명은 해시되므로 모듈 경계를 넘어 선택할 수 없기 때문이다.
// 모션 자체(키프레임)는 이 아이콘 모듈에 둔다.

const ORBIT = { cx: 12, cy: 12, rx: 10.1, ry: 4.6 };

/**
 * 옆구리(중앙 쪽)의 최소 폭 / 장축 꼭짓점의 최대 폭.
 *
 * 꼭짓점 바깥 끝 = cx + rx + max/2 = 23.05 로 24 viewBox 안에 들어온다.
 * max를 키우려면 rx를 같이 줄여야 아이콘이 잘린다.
 */
const ORBIT_WIDTH = { min: 0.35, max: 1.9 };

/**
 * 겹쳐 그릴 두 궤도. 앞의 것이 더 진하게 깔린다.
 *
 * 기울기 차 21°가 균형점이다. 11°까지 좁히면 두 링이 거의 포개져 하나의 두꺼운
 * 타원으로 보이고, 그 상태에서 중앙 도트와 합쳐지면 눈 모양으로 읽힌다.
 * 반대로 31° 이상 벌리면 두 겹이 따로 노는 인상이 된다.
 *
 * 뒤 궤도는 `dy`만큼 아래로 내려 앉힌다 — 중심을 완전히 공유하면 두 링이
 * 한 점에서 교차해 대칭으로 보이는데, 살짝 어긋나야 깊이가 생긴다.
 *
 * 상태별 값. **쉬는 상태가 가라앉은 쪽이고, hover가 불이 들어오는 쪽이다.**
 * - `dim`     쉴 때의 원반 밝기. 원반이 낮게 깔려야 흐르는 별빛이 주인공이 된다.
 * - `opacity` hover에서 되살아나는 원래 밝기. CSS가 죽었을 때의 기본값이기도 해서
 *             SVG 속성으로도 박아 둔다 — 그래야 로고가 원형 그대로 남는다.
 * - `flow`    별빛이 궤도를 한 바퀴 도는 시간. **쉴 때 기준(느린 쪽)** 이고,
 *             hover에서 재생 배속으로 두 배 빨라진다. 두 값의 비가 정수배면 같은
 *             장면이 주기적으로 되돌아온다 — 1.62(황금비 근처)라 잘 겹치지 않는다.
 * - `drift`   원반이 3D로 기우는 흔들림의 위상. 뒤 겹을 반 주기 밀어 서로 반대로
 *             기울게 한다(음수 delay = 이미 그만큼 진행한 상태로 시작).
 * - `star`    이 겹을 흐르는 별빛의 밝기 배수. 뒤 겹은 멀리 있으니 어둡다.
 */
const ORBIT_LAYERS = [
  { tilt: -24, dy: 0, opacity: 0.95, dim: 0.42, flow: "8.4s", drift: "0s", star: 1 },
  { tilt: -45, dy: 1, opacity: 0.55, dim: 0.26, flow: "13.6s", drift: "-4.5s", star: 0.7 },
];

/** 궤도 한 바퀴에 흐르는 별빛 덩어리 수 */
const FLOW_COMETS = 2;

/**
 * 별빛이 지나는 길 — 궤도선 대비 축 배율.
 *
 * 별빛을 궤도선 **위**에 두면 안 된다. 선도 아이콘 색(#fafafa)이라 흰 별빛이
 * 흰 선에 겹치는 순간 사라진다. 불투명도를 올려도 소용없다(흰색 위의 흰색은
 * 흰색). 그래서 선을 비켜 **안쪽 검은 면 위**를 지나가게 한다.
 *
 * 바깥쪽은 못 쓴다 — 꼭짓점 바깥 끝이 이미 23.05라, 더 벌리면 24 viewBox를
 * 넘어 잘린다. 안쪽으로 얼마나 들어갈지는 두 지점이 정한다:
 * 꼭짓점 쪽은 선이 두꺼워(반폭 0.95) 더 들어가야 하고, 옆구리는 선이 얇아
 * (반폭 0.175) 덜 들어가도 된다. 축 배율로 줄이면 반지름에 비례해 간격이
 * 생겨서 이 두께 차이를 대체로 따라간다.
 *
 * 0.82는 **선에 붙일 수 있는 한계**다. 이보다 키우면(0.88쯤) 별빛이 선에
 * 닿아 다시 흰색끼리 겹쳐 사라지는 구간이 생기고, 줄이면 궤도와 상관없는
 * 안쪽 링을 따로 도는 것처럼 보인다.
 */
const FLOW_TRACK = 0.82;

/**
 * 별빛 하나의 단면. 같은 지점에서 시작하는 세 겹을 포개 **혜성 꼬리**를 만든다 —
 * 짧고 굵은 겹이 머리, 길고 가는 겹이 꼬리라서 겹칠수록 밝고 굵어진다.
 *
 * 길이가 균일한 dash 하나만 쓰면 흐르는 빛이 아니라 로딩 스피너의 점선으로 읽힌다.
 * 밝기가 앞뒤로 기울어야 "어느 쪽으로 도는지"가 보인다.
 */
const FLOW_TRAIL = [
  { len: 7.4, width: 0.55, opacity: 0.18 },
  { len: 3.6, width: 0.75, opacity: 0.4 },
  { len: 1.3, width: 0.95, opacity: 1 },
];


/** 타원 위 한 점의 위치·바깥법선·반폭 */
function orbitSample(deg: number) {
  const t = (deg * Math.PI) / 180;
  const px = ORBIT.cx + ORBIT.rx * Math.cos(t);
  const py = ORBIT.cy + ORBIT.ry * Math.sin(t);

  // 타원의 바깥 법선은 (ry·cos t, rx·sin t) 방향이다 (접선의 수직).
  const nLen = Math.hypot(ORBIT.ry * Math.cos(t), ORBIT.rx * Math.sin(t)) || 1;
  const nx = (ORBIT.ry * Math.cos(t)) / nLen;
  const ny = (ORBIT.rx * Math.sin(t)) / nLen;

  // |cos t| — 꼭짓점(t=0,180)에서 1, 중앙 쪽 옆구리(t=90,270)에서 0.
  const taper = Math.abs(Math.cos(t));
  const half = (ORBIT_WIDTH.min + (ORBIT_WIDTH.max - ORBIT_WIDTH.min) * taper) / 2;

  return { px, py, nx, ny, half };
}
...
/**
 * 폭이 변하는 닫힌 링 하나. 모듈 로드 시 한 번만 계산한다.
 *
 * 바깥 고리는 정방향, 안쪽 고리는 역방향으로 감는다 — 감는 방향이 반대여야
 * 가운데가 채워지지 않고 링으로 남는다(nonzero fill rule).
 */
function orbitRing(segments = 120): string {
  const outer: string[] = [];
  const inner: string[] = [];

  for (let i = 0; i < segments; i += 1) {
    const { px, py, nx, ny, half } = orbitSample((360 * i) / segments);
    outer.push(fmt(px + nx * half, py + ny * half));
    inner.push(fmt(px - nx * half, py - ny * half));
  }

  return `M${outer.join("L")}ZM${inner.reverse().join("L")}Z`;
}

const ORBIT_PATH = orbitRing();

/**
 * 궤도 둘레. 타원 둘레는 닫힌 식이 없어서 잘게 잘라 더한다.
 *
 * 별빛은 이 길이만큼 dash를 밀어서 흐르므로, 둘레와 dash 패턴이 어긋나면
 * 한 바퀴마다 이음매가 튄다. 그래서 둘 다 여기서 한 번에 계산한다.
 */
function orbitSpan(segments = 2048): number {
  let span = 0;
  let prev = orbitSample(0);

  for (let i = 1; i <= segments; i += 1) {
    const cur = orbitSample((360 * i) / segments);
    span += Math.hypot(cur.px - prev.px, cur.py - prev.py);
    prev = cur;
  }
...
/**
 * 별빛 하나가 차지하는 길 (dash 한 주기).
 *
 * 두 축을 같은 배율로 줄인 건 닮은꼴이라 둘레도 정확히 그 배율만큼 줄어든다.
 */
const FLOW_CELL =
  Math.round(((orbitSpan() * FLOW_TRACK) / FLOW_COMETS) * 100) / 100;

/**
 * 한 바퀴로 삼을 길이. 둘레 실측값이 아니라 **반올림한 dash 주기의 정수배**다 —
 * 애니메이션이 미는 거리와 dash 주기가 정확히 맞아야 이음매 없이 순환한다.
 */
const FLOW_SPAN = String(Math.round(FLOW_CELL * FLOW_COMETS * 100) / 100);

/** 궤도를 따라 흐르는 별빛의 한 겹 (머리·중간·꼬리가 같은 시작점을 공유한다) */
const OrbitStream = ({
  layer,
  trail,
}: {
  layer: (typeof ORBIT_LAYERS)[number];
  trail: (typeof FLOW_TRAIL)[number];
}) => (
  <ellipse
    className={s.orbitFlow}
    data-orbit-flow=""
    cx={ORBIT.cx}
    cy={ORBIT.cy}
    rx={Math.round(ORBIT.rx * FLOW_TRACK * 100) / 100}
    ry={Math.round(ORBIT.ry * FLOW_TRACK * 100) / 100}
    transform={`translate(0 ${layer.dy}) rotate(${layer.tilt} 12 12)`}
    fill="none"
    stroke="currentColor"
    strokeWidth={trail.width}
    strokeLinecap="round"
    strokeDasharray={`${trail.len} ${Math.round((FLOW_CELL - trail.len) * 100) / 100}`}
    style={
      {
        animationDuration: layer.flow,
        "--orbit-span": FLOW_SPAN,
        "--orbit-star": Math.round(trail.opacity * layer.star * 1000) / 1000,
      } as CSSProperties
    }
  />
);

export const IconImpakersOrbit = ({
  size = 20,
  expanded = false,
  pulsing = false,
}: {
  size?: number;
  /** FAB 메뉴가 펼쳐진 상태 — 도트가 ✕로 바뀐다 */
  expanded?: boolean;
  /** 미확인 알림 — 도트가 맥동한다 */
  pulsing?: boolean;
}) => (
  <svg
    ...
    viewBox="0 0 24 24"
    fill="none"
    className={`${expanded ? s.orbitExpanded : ""} ${pulsing ? s.orbitPulsing : ""}`}
  >
    {/*
      기울기는 안쪽 <path>에 둔다. 흔들림(drift) CSS가 바깥 <g>의 transform을
      덮어쓰기 때문에, 여기에 rotate(-24)를 두면 애니메이션이 시작되는 순간
      기울기가 풀려 아이콘이 튄다.
    */}
    <g className={s.orbitRing} data-orbit-ring="">
      {ORBIT_LAYERS.map((layer) => (
        // 겹마다 따로 흔들려야 두 궤도 사이 간격이 벌어졌다 좁혀지며 깊이가 산다.
        <g
          key={layer.tilt}
          className={s.orbitLayer}
          data-orbit-layer=""
          style={{ animationDelay: layer.drift }}
        >
          {/*
            fill을 명시해야 한다 — 호스트 앱 보호용 `svg[fill="none"] :not([fill])`
            규칙(styles/_glass.scss의 protect-stroke-icons)이 fill 없는 자손을
            전부 fill:none으로 만들어 이 도형을 지워버린다.
          */}
          {/* SVG transform은 오른쪽부터 적용된다 — 중심 기준으로 돌린 뒤 내려앉힌다 */}
          <path
            className={s.orbitDisc}
            data-orbit-disc=""
            d={ORBIT_PATH}
            transform={`translate(0 ${layer.dy}) rotate(${layer.tilt} 12 12)`}
            fill="currentColor"
            ...
            style={
              {
                // hover 밝기. `--orbit-lit`은 hover를 끄는 감소 모션 모드에서
                // 원래 밝기로 되돌리는 용도다 (opacity 속성값으로는 못 돌아간다).
                "--orbit-lit": layer.opacity,
                "--orbit-dim": layer.dim,
              } as CSSProperties
            }
          />
          {FLOW_TRAIL.map((trail) => (
            <OrbitStream key={trail.len} layer={layer} trail={trail} />
          ))}
        </g>
      ))}
    </g>
    {/*
      두 번에 걸쳐 줄인 값 (r 1.8 → 1.3 → 1.0). 도트가 크면 궤도 hairline보다
      무거워져 아이콘이 "점을 감싼 링"으로 읽힌다. 더 줄이면 20px(모바일)에서
      지름이 2px 아래로 떨어져 피드백 지점이 아니라 얼룩처럼 보인다.
    */}
    <circle className={s.orbitDot} cx="12" cy="12" r="1" fill="currentColor" />
    <path
      className={s.orbitCross}
