...
// Exclude selectors — toolbar/popover UI elements should never be frozen
const EXCLUDE_ATTRS = [
...
const STATE_KEY = "__local_feedbacker_freeze";
...
function isExcludedFromFreeze(el: Element | null): boolean {
  if (!el) return false;
...
export function freeze(): void {
  ...
  // WAAPI — pause only RUNNING non-excluded animations and store references
  ...
  try {
    document.getAnimations().forEach((anim) => {
      ...
      if (!isExcludedFromFreeze(target)) {
        anim.pause();
...
export function unfreeze(): void {
  ...
  for (const cb of timeoutQueue) {
    _s.origSetTimeout(() => {
      ...
      try {
        ...
        console.warn("[local-feedbacker] Error replaying queued timeout:", e);
      }
    }, 0);
  }
  ...
  for (const anim of _s.pausedAnimations) {
    try {
      ...
      console.warn("[local-feedbacker] Error resuming animation:", e);
    }
  }
  _s.pausedAnimations = [];
