...
export function detectReactApp(): {
  ...
  const hasReactContainer = (document.getElementById("root") as unknown as Record<string, unknown> | null)?._reactRootContainer !== undefined;
...
/**
 * Rank a probed source path by how useful it is for debugging.
 *
 * shadcn-style shared primitives (components/ui/button.tsx) are almost never the
 * file someone wants opened — the app code that *uses* them is. The probe can't
 * see call sites at all, so the best it can do is push shared primitives down.
 *
 * Lower score = better candidate.
 */
function sourcePathRank(fileName: string): number {
  const path = fileName.toLowerCase();

  if (/(^|\/)node_modules\//.test(path)) return 4;
  // shadcn/ui primitives — shared across the whole app, rarely the answer
  if (/(^|\/)components\/ui\//.test(path)) return 3;
  // App Router entry points and domain components
  if (/(^|\/)(app|src\/app|pages|src\/pages)\//.test(path)) return 0;
  if (/(^|\/)components\//.test(path)) return 1;
  return 2;
}
 ...
 *
 * Candidates are ranked so that app code outranks shared UI primitives. Fiber
 * distance is preserved as the tie-breaker, so the nearest useful component
 * still wins within a rank.
 */
function probeSourceWalk(
  fiber: ReactFiber,
  maxDepth = 15
): SourceLocation[] {
  const results: Array<{ source: SourceLocation; distance: number }> = [];
  ...
  while (current && depth < maxDepth) {
    ...
    if (source && !seenFileNames.has(source.fileName)) {
      ...
      results.push({ source, distance: depth });
    }
  ...
  return results
    .sort((a, b) => {
      const rankDiff = sourcePathRank(a.source.fileName) - sourcePathRank(b.source.fileName);
